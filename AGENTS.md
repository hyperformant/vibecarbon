...
Enforcement happens at provisioning only, never at redeploy, backup, restore, failover, or scale: `PAID_TIERS` in `src/lib/licensing/gate.js` is derived from `requiredTierFor()` (`compose-ha`/`k8s`/`k8s-ha`), single-server Compose is free, and `requireProvisionEntitlement()` in `src/lib/licensing/index.js` is the only gate. Keys are Ed25519-verified offline in `validator.js`. A v2 key carries a paid-through date compared offline against the CLI's own stamped release date, with an optional refresh from vibecarbon.com (`vibecarbon activate -refresh`). Guards live in `tests/unit/licensing/`.
