...
Available modes: `compose`, `k8s`, and `k8s-ha`. `compose` is free on Graphite; `k8s` requires Graphene; `k8s-ha` requires Fullerene, which also covers Compose HA (`-mode compose-ha`, for providers without Kubernetes). Mode availability varies by provider; `k8s-ha` is Hetzner-only today.
