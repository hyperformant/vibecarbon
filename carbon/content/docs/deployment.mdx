...
> **Single-server Docker Compose deploys need no license.** `create`, local development (`up`, `down`), and single-server production deploys never require a license key. Provisioning a new Kubernetes environment requires Graphene; Kubernetes HA requires Fullerene, which also covers Compose HA. See [Getting Started](/docs/getting-started) for licensing details.
...
| DigitalOcean | `digitalocean` | `compose`, `k8s` | `nyc3`, `sfo3`, `ams3`, `fra1`, `lon1` | `DIGITALOCEAN_API_TOKEN` |
...
Compose HA is also available on every provider for high availability without Kubernetes; select it explicitly with `-mode compose-ha`. `k8s-ha` (multi-region Kubernetes with one-command failover) is Hetzner-only today.
