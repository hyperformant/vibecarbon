...
> **Single-server Docker Compose deploys need no license.** `create`, local development (`up`, `down`), and single-server production deploys never require a license key. Advanced deploy modes (Compose HA, Kubernetes, Kubernetes HA) require a Fullerene license. See [Getting Started](/docs/getting-started) for licensing details.

## Overview

Your application can be deployed using Docker Compose (single server, no license required), Compose HA, or Kubernetes; the latter two require a Fullerene license. Deploying for a client, white-label, or resale use case? See Agency in [Getting Started](/docs/getting-started).
