...
Activating writes `.vibecarbon.license` into the project. Commit it and share it with your team; anyone working on this project can then deploy without activating separately. A key covers every CLI release published on or before its paid-through date, so if a subscription lapses, deploys keep working until a newer release ships. Run `vibecarbon activate -refresh` to pull a renewed key once you've resubscribed.
