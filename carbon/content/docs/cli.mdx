...
Only provisioning a **new** environment into a paid deploy mode consults it.
Redeploying an existing environment, and running `backup`, `restore`,
`failover`, or `scale` on it, never requires a key. Every other command,
...
and free on every existing environment: only provisioning a *new*
environment into Kubernetes requires Graphene, or Kubernetes HA requires
Fullerene, which also covers Compose HA.
...
Activate a license key (Graphene or Fullerene). A `vc2-` key is scoped to
one project, so run `activate` from inside that project's directory; it
writes `.vibecarbon.license`, which should be committed and shared with
your team so anyone working on the project can deploy without activating
separately.
...
# Pull a renewed key for this project's stored license
vibecarbon activate -refresh
```

| Flag | Description |
|------|-------------|
| `-refresh` | Ask vibecarbon.com for a renewed key covering this project's stored license, and store it if it verifies |

License keys start with `vc-` (legacy) or `vc2-` (current). A key covers
every CLI release published on or before its paid-through date, so a
lapsed subscription keeps working on that project until a newer release
ships. Subscribe at
...
# Remove both the project key and the legacy key
vibecarbon deactivate -all
```

| Flag | Description |
|------|-------------|
| `-y` | Skip confirmation prompt |
| `-all` | Remove every stored license, project and legacy |
