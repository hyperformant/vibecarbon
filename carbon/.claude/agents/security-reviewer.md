...
- **Generated secrets stay out of git:** the JWT secret, DB password, and Supabase keys live in `.env` / `.env.local`, which are git-ignored. Flag any of them appearing in a committed file, a manifest, or a log line.
...
2. **Check this project's `AGENTS.md`.** Its "Security Rules (Mandatory)" section is the enforceable baseline: role model, auth middleware, RLS conventions, secret handling. Your review must enforce those rules, and a finding that contradicts them is wrong until AGENTS.md is corrected first.
...
8. **Check Supabase client usage carefully.** This is the #1 source of security issues in a Vibecarbon app: `c.get('supabase')` keeps RLS on, `supabaseAdmin` turns it off. The wrong client in the wrong context bypasses all data access controls.
9. **Run `npm run test:security`.** It checks the structural invariants (RLS enabled on public tables, write policies scoped by `WITH CHECK`, single-origin routing). Treat it as a floor, never a substitute for reading the diff.
