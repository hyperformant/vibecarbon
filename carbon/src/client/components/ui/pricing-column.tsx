...
/*
 * Pricing tier column. Adapted from Launch UI (MIT) for Vite: lucide
 * (CircleCheckBig) -> Tabler (IconCircleCheck), next/link -> plain <a>, and the
 * "one-time payment / plus local taxes" copy reframed for a monthly
 * subscription. Retint is automatic — `brand` maps to the template teal.
 */
        ...
        <ul className="flex flex-col gap-2">
          {features.map((feature) => (
            <li key={feature} className="flex items-center gap-2 text-sm">
              <IconCircleCheck className="text-muted-foreground size-4 shrink-0" />
              {feature}
            </li>
          ))}
        </ul>
