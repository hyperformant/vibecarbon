...
6. **A license for Kubernetes and Kubernetes HA**: single-server Docker Compose deploys need no license (Graphite); `k8s` needs a Graphene subscription, `k8s-ha` needs Fullerene, which also covers `compose-ha`.
