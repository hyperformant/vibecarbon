...
export function assessStatusJson(json: unknown, envName: string): Verdict {
  const env = (json as { environments?: Record<string, { checks?: Record<string, unknown> }> })
  ...
  if (!containers)
    return { ok: false, retryable: false, problems: [`no containers data for ${envName}`] };
